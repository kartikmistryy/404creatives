...
const Navbar = ({ linkTitles, links }) => {
  ...
  return (
    <div className={`${chivo.className}  w-full h-22  z-10`}>
      <div
          style={{ transition: "all ease 0.3s" }}
          className={`absolute top-0 right-0 w-full h-[100vh] flex flex-col bg-[#f2f2f2] text-[#252222] items-end justify-start   origin-bottom ${
            showMenu
              ? "opacity-100 translate-y-0 z-[99] pointer-events-auto"
              : "opacity-0 translate-y-10 z- pointer-events-none"
          } `}
        >
          <div className="flex flex-col w-full h-full max-w-[1600px] mx-auto items-end justify-start md:px-12 px-5 md:py-8 py-5 gap-10">
            <button
              onClick={() => {
            ...
            <ul className={`flex flex-col gap-5 items-end ${chivo.className}`}>
              <li className="font-normal md:text-[1.1rem] text-2xl">
              ...
              {linkTitles &&
                linkTitles.map((title, index) => {
                  return (
                    <li
                      key={index}
                      className="font-normal md:text-[1.1rem] text-2xl"
                    >
      ...
      <div className="flex absolute top-0 right-0 left-0 w-full h-20 max-w-[1600px] mx-auto">
        ...
        <div className="w-full h-fit">
          <div className="w-full md:h-[10vh] h-16 md:px-10 px-5 flex items-center justify-start">
            <div className="w-full md:h-[10vh] h-16 flex items-center justify-between">
              <Link
                href="/"
                className="font-black w-fit flex items-center justify-start md:w-[200px]"
              >
                <Image
                  width={100}
                  height={100}
                  src="/Logo/404.png"
                  alt="logo"
                  className="md:w-14 md:h-14 w-10 h-10 md:mb-4 mb-2"
                />
              </Link>
              ...
              <span className="flex items-center justify-between md:w-[200px] w-fit md:gap-10 gap-0">
                <Link
                  href="/Shop"
                  className="font-mono md:flex hidden items-center justify-center md:py-[3px] py-[5px] md:px-[40px] px-[30px] rounded-3xl border-2 border-[#252222] font-medium md:text-[1.1vw] text-lg md:scale-100 scale-75"
                >
                  SHOP
                </Link>
                <button
